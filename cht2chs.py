...
CHT_COUNTRY_CODES = [ "Base", "zh-Hant" ]
...
def cht_to_chs(src):
    ...
    for i, phrase in enumerate(phrase_cht):
        src = src.replace(phrase, phrase_chs[i])
...
localizable_cht = get_string_map(BASE_PATH + "/Base.lproj/Localizable.strings")

with open("Utils/i18n.swift", "w") as dest:
        dest.write(
        """//
//  i18n.swift
//  bbmap
...
localizable_cht = get_string_map(BASE_PATH + "/zh-Hant.lproj/Main.strings")        
for code in CHS_COUNTRY_CODES:
    path = BASE_PATH + "/%s.lproj/Main.strings" % (code)
    mkdir(path)
    with open(path, "w") as dest:
        for key, value in sorted(localizable_cht.items()):
            if key in strings_chs:
...
localizable_cht = get_string_map(BASE_PATH + "/Base.lproj/InfoPlist.strings")                
for code in CHS_COUNTRY_CODES:
    path = BASE_PATH + "/%s.lproj/InfoPlist.strings" % (code)
    mkdir(path)
    with open(path, "w") as dest:
        for key, value in sorted(localizable_cht.items()):
            if key in strings_chs:
